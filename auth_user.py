...
def main() -> int:
    ...
    print(f"  {url}")
    print()
    print(f"IMPORTANT — copy that URL into an INCOGNITO browser window signed")
    print(f"into Slack as {args.email}.")
    print()
    print("Why incognito: Slack's OAuth URL is identity-blind — it grants a token")
    print("for whichever Slack user is signed into the browser that loads it. Your")
    print("default browser is probably signed in as Jennifer (or your primary demo")
    print("persona), so loading the URL there would issue HER token, not the")
    print("persona's. The incognito window gives you a clean session signed in as")
    print("the persona only. The verification step below will catch and reject the")
    print("wrong-user case, but incognito avoids the round trip.")
    print()
    if args.no_open:
    ...
    print()
    print(f"Verifying the captured token belongs to {args.email}...")
    ...
    if not ok:
        ...
        print(detail)
        print()
        print("Most likely causes (check each before re-running):")
        print("  1. The OAuth URL was opened in your default browser instead of")
        print("     the incognito window.")
        print("  2. The auto-opened tab in your default browser was clicked")
        print("     through to 'Allow' before you got to incognito (re-run with")
        print("     --no-open to suppress the auto-open).")
        print("  3. The incognito window was signed into Slack as the wrong user")
        print("     (or wasn't signed in at all and prompted you to sign in there).")
        print(f"  4. The --email value ({args.email!r}) doesn't match a real Slack")
        print("     account in this workspace.")
        print()
        print(f"To retry: open a fresh incognito window, sign into Slack as")
        print(f"{args.email}, then re-run this command.")
        return 2
